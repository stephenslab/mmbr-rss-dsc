...
The correlation matrix of Y using pairwise complete observations:
```{r}
library(corrplot)
corrplot(cov(Y, use='pairwise.complete.obs'), method='color', type='upper', tl.col="black", tl.srt=45, is.corr = FALSE)
```

The covarince matrix of Y using FLASH:
```{r}
...
| model 3 | `r res3_L3$result_time['elapsed']` | `r res3_L3$result$walltime['elapsed']` | `r res1$result$niter` |
| model 4 | `r res4_L3$result_time['elapsed']` | `r res4_L3$result$walltime['elapsed']` | `r res1$result$niter` |
...
```{r}
univariate_res = lapply(1:ncol(Y), function(i) susieR:::univariate_regression(X,Y[,i]))
bhat = do.call(cbind, lapply(1:ncol(Y), function(i) univariate_res[[i]]$betahat))
shat = do.call(cbind, lapply(1:ncol(Y), function(i) univariate_res[[i]]$sebetahat))
zhat = bhat/shat
zhat[is.na(zhat)] = 0
R = cor(X)
res_rss = mmbr::msusie_rss(zhat, R, residual_variance = cov2cor(resid_Y), prior_variance=prior)
susie_plot(res_rss, y='PIP')
```

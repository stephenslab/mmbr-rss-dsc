...
out = dscrutils::dscquery('output/ukb_rss', targets = c('simulate', 'simulate.eff_mode',
                                                      'mnm_suff','mnm_suff.resid_method', 'mnm_suff.DSC_TIME',
                                                      'mnm_rss', 'mnm_rss.resid_method','mnm_rss.DSC_TIME', 
                                                      'susie_uni', 'susie_uni.estimate_residual_variance', 'susie_uni.DSC_TIME',
                                                      'mvsusie_scores', 'mvsusie_scores.total', 'mvsusie_scores.valid', 
                                                      'mvsusie_scores.size', 'mvsusie_scores.purity', 'mvsusie_scores.top', 
                                                      'mvsusie_scores.n_causal', 'mvsusie_scores.included_causal', 
                                                      'mvsusie_scores.overlap_var', 'mvsusie_scores.overlap_cs',
                          ...
                          module.output.files = c('simulate', 'mnm_suff','mnm_rss', 'susie_uni'), 
                          ignore.missing.files = TRUE, 
                          groups = c('mnm:', 'mnm_suff: mnm_suff_oracle, mnm_suff_ed'))

saveRDS(out, 'result/ukb_rss_20210313/ukb_rss_20210313.rds')
